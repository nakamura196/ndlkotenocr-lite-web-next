...
export default function Home() {
  ...
  const fetchAndConvertIiifManifest = useCallback(async (manifestUrl: string) => {
    ...
    try {
      ...
      if (convertedManifest?.items) {
          ...
          if (canvas.items?.[0]?.items) {
            canvas.items[0].items.forEach((annotation: any) => {
              if (annotation.body?.id) {
                ...
                console.log(`Found image: ${label} - ${imageUrl}`)
              }
            })
          }
        })
        
        console.log(`Found ${imageInfos.length} images in manifest`)
        ...
        console.log('No items found in manifest')
  ...
  const handleIiifSubmit = useCallback((e: React.FormEvent) => {
    e.preventDefault()
    console.log('IIIF form submitted with URL:', iiifUrl)
    if (iiifUrl.trim()) {
  ...
  // サーバーサイド画像処理
  const processImagesOnServer = useCallback(async () => {
    if (selectedImages.length === 0) return

    setIsProcessing(true)
    setProgress(0)
    setLoadingMessage('サーバーでOCR処理中...')
    ...
    try {
      ...
      for (let i = 0; i < selectedImages.length; i++) {
        ...
        setImageProgress(0)
        setLoadingMessage(`画像 ${i + 1}/${selectedImages.length} をサーバーで処理中...`)
      ...
      alert(`サーバーでの画像処理中にエラーが発生しました: ${error instanceof Error ? error.message : '不明なエラー'}`)
    } finally {
    ...
    try {
      ...
      if (processingMode === 'server') {
        ...
        for (let i = 0; i < thumbnails.length; i++) {
          ...
          setImageProgress(0)
          setLoadingMessage(`画像 ${i + 1}/${thumbnails.length} をサーバーで処理中...`)
          setProgress(40 + ((i + 1) / thumbnails.length) * 60) // 40%から100%まで
          
          const formData = new FormData()
          formData.append('image', thumbnails[i].file)
  ...
  }, [selectedIiifImages, processingMode, initializeOCR, createThumbnail, shouldStop])

  // 統合処理関数（モードに応じて処理を振り分け）
  ...
  }, [processingMode, processImagesOnServer, processImagesOnClient])
  ...
  }, [processedResults, selectedImages, iiifManifest, iiifUrl])
